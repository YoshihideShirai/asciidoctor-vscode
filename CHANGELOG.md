...
* Add `asciidoc.preview.defaultStyle` to pick the built-in stylesheet used when `asciidoc.preview.style` is empty and the document does not set `stylesheet`/`stylesdir`: `vscode` (the VS Code theme-integrated stylesheet), `asciidoctor` (the plain Asciidoctor.js stylesheet), `antora` (a new stylesheet layering Antora Default UI fonts, colors and document block styles — headings, admonitions, code blocks, tables, quotes — on top of the Asciidoctor default stylesheet), and `github` (a new stylesheet approximating GitHub's Markdown rendering, built from Primer's functional color tokens and applied over the Asciidoctor default stylesheet; it matches light or dark to the active VS Code color theme the same way the `vscode` style already does, rather than needing a separate light/dark choice). The setting defaults to *Automatic* (an empty value, distinct from every real style — so that explicitly picking `vscode` from the Settings UI dropdown always registers as a change from what was already shown, rather than the dropdown already displaying `vscode` as the inherited default and silently discarding a re-selection of it), which follows the deprecated `asciidoc.preview.useEditorStyle` boolean, so existing configurations keep working. When Antora support is active for a document and no style was explicitly chosen — including when the style only resolved through `useEditorStyle`, since neither of its values expresses an opinion about Antora — the preview now auto-selects the `antora` style, per the "Preview Integration" section of the Antora support spec (an explicit `defaultStyle` choice, including an explicit `vscode`, always wins). `asciidoc.preview.lineHeight` is honoured by the `antora` style's reading area, which previously always rendered at a fixed line height regardless of the setting. Mermaid diagrams now render with a light or dark theme that matches the resolved preview style (previously they always followed the VS Code color theme, so e.g. `github` produced light diagram boxes on a dark page when the active VS Code theme was light). The `antora` and `github` stylesheets no longer inherit Asciidoctor's decorative oversized quote glyph on blockquotes, which clashed with both design systems; `github` now correctly themes every table row (not just the header) instead of leaving dark-mode tables with unreadable near-black text on a white background; `github`'s table of contents (the `toc: left`/`toc: right` sidebar and a `toc::[]` macro box) now themes correctly too — `asciidoctor-default.css` styles each with a rule more specific than the plain `#toc` selector `github.css` was overriding, so both silently kept Asciidoctor's stock light-gray canvas in dark mode; and `github`'s admonitions were re-checked against github.com's actual rendering of an `.adoc` file (not just its Markdown Alert callouts, which AsciiDoc admonitions don't map to) — GitHub applies no admonition-specific styling at all, so they now match that bare presentation (a plain bordered table, plain-text caption, no icon or accent color) instead of the boxed-and-colored Alert look adapted from Markdown. Authors who want the colored, emoji-captioned Alert look specifically on github.com can still opt in per the long-standing `ifdef::env-github[]` `*-caption` convention (see `examples/showcase.adoc`), which only takes effect on github.com itself. Several further `github` fidelity gaps found by comparing the preview against github.com's actual rendering of the same document are also fixed: the author/revision byline under the title was near-black and unreadable on the dark canvas; the table of contents kept a boxed border and background that GitHub's own unstyled TOC-like navigation doesn't have; `h5` was incorrectly muted like `h6` (GitHub's heading scale only mutes `h6`); the `[.lead]` role (and the automatic "lead" first preamble paragraph) no longer renders larger or in a near-black color than a plain paragraph, since GitHub has no Markdown equivalent to that AsciiDoc-only convention; and the admonition label cell now shrinks to the caption's width with a collapsed hairline border (`border-collapse: collapse`) instead of stretching wide with a doubled-up border, matching GitHub's own `<table>` layout. Heading sizes and margins were also off: `h1`–`h6` were sized in `em` off `body`'s font-size (`--asciidoc-font-size`, which defaults to the 14px editor font size) while paragraph text is pinned to a fixed `1.0625rem` off the 16px root — two disconnected baselines that shrank every heading-to-body ratio below GitHub's (e.g. `h2` came out ~1.24× body instead of GitHub's 1.5×) — and their `margin-top`/`margin-bottom` inherited the default stylesheet's relative `1em`/`.5em` off that same small base instead of GitHub's fixed 24px/16px. Both are now sized/spaced in `rem`, anchored to the same root paragraph text already uses. The horizontal rule `asciidoctor-default.css` draws between consecutive top-level sections (`.sect1 + .sect1`) is also dropped — GitHub doesn't separate sections that way. Every block title/caption (image captions, example/listing/table titles, list titles, …) shares one `asciidoctor-default.css` selector that set a decorative serif italic font and a reddish-brown color; `github` had already muted and bolded it instead, still leaving the serif font-family untouched — GitHub renders a `.title` div as plain, full-contrast body text, so all of it (including `font-family`) is now neutralized to match
* Further `github` style fidelity fixes, continuing the same github.com-comparison process: quotes and verses no longer use Asciidoctor's italicized, justified, oversized (1.15rem vs. the 1.0625rem body paragraphs get) treatment, and a `[verse]` block — which Asciidoctor renders as a literal `<pre>` — now picks up the same monospace, boxed treatment GitHub's generic `<pre>` styling gives it, instead of fighting it with a sans-serif override; the blockquote gained `padding-left` (text sat flush against the left border) and the attribution line moved from right-aligned/muted/undersized to left-aligned, full-contrast, and body-sized, sitting close to its quote with more room before the next block, and the `<cite>` line (e.g. a work's title) had its own near-black color (and default browser italic) reset the same way. Example and sidebar blocks are unstyled entirely (no border/background/padding) — like admonitions and the TOC, GitHub applies no CSS to these AsciiDoc-only constructs — and a sidebar's title no longer sizes like an `h3`, since GitHub has nothing to size it against. Every code block (plain or syntax-highlighted) now sits on one `#151b23` (dark) / `#f6f8fa` (light) canvas: the highlight.js output (`<pre class="highlightjs highlight">`) wasn't matched by the previous highlighted-vs-plain split at all (a two-token class list, not the `"highlight "`-prefixed string the selectors checked for), and separately, asciidoctor-default.css's blanket `.vscode-dark code, .vscode-dark kbd` rule (meant for the 'vscode' style's inline code) is more specific than a highlight.js theme's own background and was winning over both. `kbd` text, the `+` between chained keys (`.keyseq`), and `.menuseq`/`.menuref` (a UI menu path) were all near-black and unreadable on the dark canvas; `.menuseq` is now also bold, matching GitHub. Tables: the header row no longer gets a filled background (GitHub bolds it and nothing else), a `<th>` inside `tbody` (an AsciiDoc `h`-column-spec "header-style" cell) likewise loses its shaded background while keeping full-contrast bold text (its content is wrapped in a `<p>` that carried its own separate near-black color, unlike a thead `<th>`'s plain text), every table now stripes even body rows the way GitHub does by default (previously only tables opted into Asciidoctor's `%stripes-even` option or similar got it), inline code that is a table cell's only content keeps its usual pill background instead of asciidoctor-default.css stripping it, and a table no longer stretches to the full column width by default (Asciidoctor's `stretch` class) — GitHub always sizes a table to its content. Finally, a `.sect1 + .sect1 { border-top }` follow-up: `.sect1`'s own `padding-bottom` (1.25em on screens ≥768px) is zeroed out too, closing the remaining gap between sections
...
* Deprecate the `asciidoc.preview.useEditorStyle` setting in favor of `asciidoc.preview.defaultStyle`, which offers the same VS Code/Asciidoctor.js choice plus the new `antora`, `github-light` and `github-dark` styles. The setting is marked deprecated in the VS Code settings UI (`markdownDeprecationMessage`, all locales); its previous boolean value is still honoured whenever `defaultStyle` is left unset
...
* Simplify `AsciidocPreviewConfiguration.getDefaultStyle()`: it now reads the resolved `preview.defaultStyle`/`preview.useEditorStyle` values through `WorkspaceConfiguration.get()` instead of manually re-deriving them from `inspect()`'s workspaceFolder/workspace/global/default fields, using `inspect()` only to detect whether the setting was explicitly configured (needed to prefer the new setting over the deprecated one when both resolve to a value, since `get()` alone can't distinguish an explicit default from an unset one). No behaviour change
