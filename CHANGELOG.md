...
* Add `asciidoc.antora.siteManifestPath` to resolve `xref:` links (completion, "Go to Definition", preview) to pages published in remote-only Antora components via a `site-manifest.json` generated by [Antora Atlas](https://gitlab.com/antora/atlas) (#1181)
* Add `antora` and `github` preview styles alongside `vscode`/`asciidoctor`, selectable via the new `asciidoc.preview.defaultStyle` setting; the `antora` style is auto-selected when Antora support is active, and Mermaid diagrams now follow the resolved preview style instead of always the VS Code theme
* Improve fidelity of the new `antora` and `github` preview styles against their real-world references (Antora Default UI and github.com), covering headings, code blocks, tables, quotes, admonitions, captions and the table of contents
* Resolve VS Code path variables (`${workspaceFolder}`, `${userHome}`, `${env:NAME}`, …) consistently across preview attributes and PDF export settings, instead of only the bare `${workspaceFolder}` in some places (#1154)

### Bug fixes

* Fix `asciidoc.extensions.registerWorkspaceExtensions` failing to load workspace `.asciidoctor/lib` extensions since 4.0.0's move to native ESM (#1179)
* Render `image::foo.svg[opts=interactive]` in the preview by allow-listing it in the `frame-src` Content Security Policy directive
* Make `asciidoc.antora.showEnableAntoraPrompt` actually take effect, and show the "enable Antora support?" prompt for the document that triggered activation, not just ones opened afterward
* Keep the Antora content catalog alive when the workspace aggregates the same component version twice or declares a non-string `version`, instead of breaking every Antora feature
* Log Antora resolution failures (dropped/merged `antora.yml` files, unmatched documents) to the "Asciidoctor" output channel instead of failing silently
* Fully reload the preview when an edit changes the webview "shell" (e.g. toggling `:stem:` or `:source-highlighter:`), instead of leaving it in an inconsistent partially-updated state
* Fix STEM rendering in the preview on a machine without internet access by bundling the mhchem font extension instead of fetching it from a CDN (#1160)

### Deprecations

* Deprecate `asciidoc.preview.useEditorStyle` in favor of `asciidoc.preview.defaultStyle`, which adds the new `antora`, `github-light` and `github-dark` styles
* Deprecate `asciidoc.useWorkspaceRootAsBaseDirectory`; use an `.asciidoctorconfig` file anchored on `{asciidoctorconfigdir}` instead (#1155)

### Documentation

* Clarify on the Antora page that the "remote content sources" limitation only concerns Antora playbooks, not remote-development setups like VS Code Remote - SSH, and document that the enable-support prompt is opt-in
* Clarify how relative include/image paths resolve when `asciidoc.useWorkspaceRootAsBaseDirectory` is enabled (#700)

### Infrastructure

* Migrate the codebase to TypeScript `"strict": true` (`noImplicitAny`, `strictNullChecks`, `strictPropertyInitialization`), fixing a couple of latent null-handling bugs surfaced along the way
* Fix `antora.yml` parsing in the web extension, where `Uint8Array` was passed straight to `yaml.load` instead of being decoded first
* Shrink the packaged VSIX from ~29 MB to ~14 MB by trimming `.vscodeignore` re-includes and the bundled Mermaid assets to what's actually needed at runtime
* Simplify `AsciidocPreviewConfiguration.getDefaultStyle()` to read resolved settings via `WorkspaceConfiguration.get()` instead of manually re-deriving them (no behaviour change)
* Update dependencies, notably `@asciidoctor/core` to 4.0.6, `mermaid` to 11.16.0, `mathjax` to 4.1.3, and `js-yaml` to v5
