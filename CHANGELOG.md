...
* Add `asciidoc.preview.defaultStyle` to pick the built-in stylesheet used when `asciidoc.preview.style` is empty and the document does not set `stylesheet`/`stylesdir`: `vscode` (the VS Code theme-integrated stylesheet, the existing default), `asciidoctor` (the plain Asciidoctor.js stylesheet), `antora` (a new stylesheet layering Antora Default UI fonts, colors and document block styles — headings, admonitions, code blocks, tables, quotes — on top of the Asciidoctor default stylesheet), `github-light` and `github-dark` (two new stylesheets approximating GitHub's Markdown rendering, built from Primer's functional color tokens and applied over the Asciidoctor default stylesheet). This replaces the boolean `asciidoc.preview.useEditorStyle`, which is now deprecated but still honoured when `defaultStyle` is left unset, so existing configurations keep working. When Antora support is active for a document and no style was explicitly chosen, the preview now auto-selects the `antora` style, per the "Preview Integration" section of the Antora support spec (an explicit choice, including an explicit `vscode`, always wins). `asciidoc.preview.lineHeight` is honoured by the `antora` style's reading area, which previously always rendered at a fixed line height regardless of the setting. Mermaid diagrams now render with a light or dark theme that matches the resolved preview style (previously they always followed the VS Code color theme, so e.g. `github-dark` produced light diagram boxes on a dark page when the active VS Code theme was light). The `antora` and `github-light`/`github-dark` stylesheets no longer inherit Asciidoctor's decorative oversized quote glyph on blockquotes, which clashed with both design systems, and `github-light`/`github-dark` now correctly theme every table row (not just the header): a `asciidoctor-default.css` rule sets table cell text color directly, which previously always won over the new stylesheets' color regardless of load order, leaving `github-dark` tables with unreadable near-black text (and inline code) on a white table background
