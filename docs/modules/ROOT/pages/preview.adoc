...
* `asciidoc.preview.defaultStyle` picks the built-in stylesheet used when neither `asciidoc.preview.style` nor the document's own `stylesheet`/`stylesdir` attributes are set: `vscode` (the default; follows the VS Code editor theme, `workbench.colorTheme`), `asciidoctor` (the plain Asciidoctor.js stylesheet), `antora` (approximates the Antora Default UI), or `github` (approximates GitHub's Markdown rendering; like `vscode`, it follows the active VS Code color theme rather than needing a separate light/dark choice).
* When xref:spec:antora.adoc[Antora support] is active for a document and `defaultStyle` is left at its default (`vscode`), the preview automatically uses the `antora` style instead. Choosing any style explicitly always overrides this.
...
`asciidoc.preview.useEditorStyle` is deprecated in favor of `asciidoc.preview.defaultStyle`; its value is still honored as long as `defaultStyle` is left unset.
...
A `toc: left` or `toc: right` document attribute renders the table of contents as a *sidebar on the right* of the preview, in both the editor style and the default Asciidoctor style.

[NOTE]
====
`toc: left` is *not* supported as a left-hand sidebar. A left sidebar overlaps the document text in the preview instead of reserving a column for it, so a left-positioned table of contents is placed on the right instead -- the same as `toc: right` -- where it lays out correctly.
====

When the preview is too narrow to show the sidebar, the table of contents collapses inline below the document title, the same way a `:toc:` (auto) table of contents is always displayed. A table of contents placed in the body with the `toc::[]` macro keeps its boxed styling wherever it appears.
